...
export function Step2PersonalDetails({
  ...
  const [digilockerSession, setDigilockerSession] =
    useState<DigilockerSession | null>(null);
  ...
  const isAgeBlocked =
    !!String(dateOfBirth || "").trim() && !isAgeEligible(dateOfBirth);
  ...
  const handleFileChange =
    (field: keyof PersonalDetailsForm) => (file: File | null) => {
  ...
  const onValidSubmit = async (data: PersonalDetailsForm) => {
    ...
    if (!isAgeEligible(data.dateOfBirth)) {
      ...
      toast.error(
        `Age must be between ${MIN_ELIGIBLE_AGE} and ${MAX_ELIGIBLE_AGE} years. The application cannot be submitted.`,
      );
      return;
    }
    ...
    if (panName && aadhaarName) {
      ...
      if (!hasOverlap) {
        ...
        toast.error(
          "Name on PAN and Aadhaar do not match. The application cannot be submitted.",
        );
        return;
      }
    }
    ...
    if (
      ...
      verifiedAadhaarRef.current !== aadhaarDigits
    ) {
      toast.error(
        "Please verify your Aadhaar with DigiLocker before continuing.",
      );
      return;
    }
    ...
    try {
      ...
      if (response && response.data) {
        ...
        if (p.full_name) {
          ...
          if (p.middle_name)
            setValue("middleName", p.middle_name, { shouldValidate: true });
        }

        if (p.gender) {
          const g = p.gender.toUpperCase();
          setValue("gender", g === "M" || g === "MALE" ? "Male" : "Female", {
            shouldValidate: true,
          });
        }
        ...
        try {
          ...
          console.error(
            "Failed to push lead to Kylas after PAN verification",
            kylasErr,
          );
        }
      }
      ...
      if (
        ...
        errorMsg.includes("another account")
      ) {
        toast.error(
  ...
  const verifyAadhaarNumber = React.useCallback(
    async (digits: string): Promise<boolean> => {
      ...
      try {
        ...
        if (response && response.success === false) {
          throw new Error(
            response.message || "Please enter a valid Aadhaar card number.",
          );
        }
        ...
        if (
          ...
          msg.includes("already validated")
        ) {
          verifiedAadhaarRef.current = digits;
          setAadhaarStatus("valid");
          return true;
        ...
        setAadhaarStatus("invalid");
        setAadhaarError(
          e.message || "Please enter a valid Aadhaar card number.",
        );
        return false;
      }
    },
  ...
  const handleDigilockerSubmit = async () => {
    ...
    if (digits.length !== 12) {
      traceStop(
        `the aadhaar number is ${digits.length} digits, not 12 — nothing was sent`,
      );
      return;
    }
    ...
    trace(
      `step 2/7 — consent window ${popup ? "opened" : "REFUSED by the browser"}`,
    );
    ...
    try {
      ...
      if (!session) {
        ...
        traceStop(
          "no session, so DigiLocker was never opened",
      ...
      if (!popup) {
        traceStop(
          "the consent url is ready but the window was blocked — waiting for the customer to open it manually",
        );
        toast.error(
  ...
  const handleDigilockerComplete = async (
    ...
    if (status !== "success") {
      traceStop(
        "the attempt failed, so the aadhaar validate call is being skipped",
      );
    ...
    if (profile) {
      trace(
        "step 7/7 — auto-filling the form from the DigiLocker record",
        profile,
      );
      ...
      if (profile.dob && !String(watch("dateOfBirth") || "").trim()) {
        ...
        if (iso) {
          ...
          traceStop(
            `could not read the date of birth "${profile.dob}" — leaving the field empty`,
          );
        }
      }
      ...
      if ((gender === "MALE" || gender === "FEMALE") && !watch("gender")) {
        setValue("gender", gender === "MALE" ? "Male" : "Female", {
          shouldValidate: true,
        });
      }
    }
    ...
    if (
      digits &&
      digits !== String(watch("aadhaarNumber") || "").replace(/\D/g, "")
    ) {
      setValue("aadhaarNumber", digits, { shouldValidate: true });
    }

    trace(
      "consent confirmed — now calling the aadhaar validate endpoint",
      digits,
    );
    ...
    traceStop(
      "consent went through but the validate endpoint rejected the number",
      aadhaarError ?? "",
    );
    setDigilockerStatus("failed");
    toast.error(
      aadhaarError || "Aadhaar could not be validated. Please try again.",
    );
  };
  ...
  return (
    <form
      ...
      className={cn("space-y-6", "form-fade-in", "pb-4")}
    >
      {/* Header element */}
      <div
        className={cn(
      ...
      >
        ...
        <h2 className={cn("text-xl", "font-bold", "text-[#1c2b4f]")}>
          Personal Details
        </h2>
      </div>
              ...
              render={({ field }) => {
                return (
                  ...
                  >
                    ...
                    <InputOTPSeparator
                      className={cn("scale-[0.8]", "mx-0", "px-0")}
                    />
                    <InputOTPGroup className="flex-1">
                      <InputOTPSlot
                        index={5}
      ...
      <div className={cn("w-full", "mt-4")}>
        ...
        <Controller
          ...
          render={({ field }) => {
            const slotBase =
              "w-full h-10 sm:h-12 lg:h-14 text-sm sm:text-base lg:text-lg transition-colors duration-200";
            const slotClass =
              aadhaarStatus === "valid"
        ...
        <div className="mt-3">
          {digilockerStatus === "verified" ? (
            ...
            >
              <ShieldCheck
                className={cn("w-5", "h-5", "text-green-600", "shrink-0")}
              />
      ...
      <div className={cn("w-full", "mt-4")}>
        ...
        <div className="relative">
          ...
          <Input
            ...
            placeholder="Enter exactly as printed on Aadhaar"
          />
        </div>
        {errors.aadhaarName && (
          <p className={cn("text-red-500", "text-sm", "mt-1")}>
        ...
        <div className={cn("flex", "items-start", "space-x-3", "mb-4")}>
          ...
          <div>
            <h2 className={cn("text-[17px]", "font-bold", "text-[#1c2b4f]")}>
              ...
              <span
                className={cn("text-gray-400", "font-medium", "text-[13px]")}
              >
                (Address is mandatory to proceed)
              </span>
            </h2>
          </div>
        </div>
      ...
      <div className={cn("pt-4", "border-t", "border-gray-100")}>
        ...
        <div className={cn("grid", "grid-cols-1", "md:grid-cols-2", "gap-4")}>
          ...
          >
            <FileBadge2
              className={cn("w-8", "h-8", "text-blue-300", "mx-auto", "mb-2")}
            />
            <div className={cn("text-sm", "font-bold", "text-[#1c2b4f]")}>
              PAN Card Upload <span className="text-red-500">*</span>
            </div>
            <div className={cn("text-[10px]", "text-gray-500", "mb-3")}>
              JPG, PNG or PDF
              <br />
              (Max 5MB)
            </div>
            ...
            {errors.panImage && (
              ...
              >
                {(errors.panImage as any).message || "Required"}
              </p>
            )}
          </div>
          ...
          >
            ...
            <div className={cn("text-[10px]", "text-gray-500", "mb-3")}>
              ...
              (Max 5MB)
              <br />
              Note: Please upload a single PDF containing both side Aadhaar
              image
            </div>
            <FileUpload
              accept=".jpg,.jpeg,.png,.pdf"
            ...
            {errors.aadhaarImage && (
              ...
              >
                {(errors.aadhaarImage as any).message || "Required"}
              </p>
            )}
          </div>
          ...
          >
            ...
            <div className={cn("text-sm", "font-bold", "text-[#1c2b4f]")}>
              salary slip (last 3-month salary slip){" "}
            ...
            {errors.salarySlipImage && (
              ...
              >
                {(errors.salarySlipImage as any).message || "Required"}
              </p>
            )}
          </div>
          ...
          >
            ...
            <div className={cn("text-sm", "font-bold", "text-[#1c2b4f]")}>
              Bank Statement (latest 6-month Bank Statement){" "}
              <span className="text-red-500">*</span>
            </div>
            <div className={cn("text-[10px]", "text-gray-500", "mb-3")}>
              PDF Only
              ...
              (Max 10MB)
              <br />
              Note: All bank statements should be unprotected
            </div>
            <FileUpload
              accept=".pdf"
            ...
            {errors.bankStatementImage && (
              ...
              >
                {(errors.bankStatementImage as any).message || "Required"}
              </p>
            )}
          </div>
        </div>
        ...
        <div className={cn("flex", "flex-col")}>
          ...
          >
            ...
            <span className={cn("text-sm", "text-gray-700", "leading-relaxed")}>
              ...
              , including loan terms, charges, and disclosures, and hereby
              expressly consent to and agree to be legally bound by the same in
              accordance with applicable RBI regulations.{" "}
              <span className="text-red-500">*</span>
            </span>
          </label>
          {errors.consentOne && (
            <p className={cn("text-red-500", "text-[10px]", "ml-7", "mt-1")}>
              {errors.consentOne.message as string}
            </p>
          )}
        </div>
        ...
        <div className={cn("flex", "flex-col")}>
          ...
          >
            ...
            <span className={cn("text-sm", "text-gray-700", "leading-relaxed")}>
              ...
              communications through electronic modes, including WhatsApp, SMS,
              and email, in accordance with applicable laws and regulations.{" "}
              <span className="text-red-500">*</span>
            </span>
          </label>
          {errors.consentTwo && (
            <p className={cn("text-red-500", "text-[10px]", "ml-7", "mt-1")}>
              {errors.consentTwo.message as string}
            </p>
          )}
        </div>
      </div>
      ...
      {isAgeBlocked && (
        ...
        >
          <p className={cn("text-sm", "text-red-800", "font-medium")}>
            As per your PAN/Aadhaar records you are {applicantAge} years old.
            Loans are available only to applicants aged {MIN_ELIGIBLE_AGE} to{" "}
            {MAX_ELIGIBLE_AGE} years, so this application cannot be submitted.
          </p>
        </div>
      )}
      ...
      {showNameMismatch && (
        ...
        >
          <p className={cn("text-sm", "text-red-800", "font-medium")}>
            The name on your Aadhaar card does not match the name on your PAN
            card. Please verify your PAN and Aadhaar details — this application
            cannot be submitted until the names match.
          </p>
        </div>
      )}
      ...
      <div className="pt-6">
        ...
        <div className={cn("text-center", "mt-4")}>
          ...
          >
            <Lock className={cn("w-3", "h-3", "mr-1", "text-green-600")} /> Your
      ...
      <Dialog open={showAgeAlert} onOpenChange={setShowAgeAlert}>
        <DialogContent className={cn("sm:max-w-[440px]", "rounded-2xl")}>
          <DialogHeader>
            ...
            <DialogDescription
              className={cn("text-center", "text-gray-600", "pt-2")}
            >
              {applicantAge !== null
              ...
              Loans are available only to applicants aged {MIN_ELIGIBLE_AGE} to{" "}
              {MAX_ELIGIBLE_AGE} years, so we are unable to proceed with this
              application.
            </DialogDescription>
          </DialogHeader>
