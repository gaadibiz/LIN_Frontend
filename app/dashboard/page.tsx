...
function ReloanFlow() {
  ...
  const [eligibilityStatus, setEligibilityStatus] = React.useState<
      ...
      try {
        ...
        if (res && res.profile) {
          ...
          updateFormData("basicDetails", {
            ...
            purposeOfLoan: "",
            occupation:
              p.employment?.employmentType === "SALARIED"
                ? "Salaried"
                : p.employment?.employmentType === "SELF_EMPLOYED"
                  ? "Self Employed"
                  : "Salaried",
            ...
            currentAddressType:
              p.address?.currentAddressType || "Owner(Self or Family)",
  ...
  const handleEligibilitySubmit = async (data: EligibilityForm) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:5000"}/api/loans/check-eligibility`,
        {
          method: "POST",
          headers: {
  ...
  const handleDocumentVerificationSubmit = async (
    ...
    try {
      ...
      try {
        ...
        if (
          data.panImage &&
          data.panImage instanceof File &&
          data.panImage.size > 0
        ) {
          await apiClient.uploadDocument("PAN", data.panImage);
        }
        if (
          data.aadhaarImage &&
          data.aadhaarImage instanceof File &&
          data.aadhaarImage.size > 0
        ) {
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-xl font-bold text-red-600">
            {internalStep === 1 ? "Check Eligibility" : "Upload Documents"}
          </h3>
      ...
      <div className="space-y-6">
        {applicationBlock ? (
          ...
          <div className="w-full py-8 flex flex-col items-center">
            ...
            <h3 className="text-xl font-extrabold text-[#312c5b] mb-3 text-center">
              Application Not Approved
            </h3>
            <p className="text-[#6b7280] mb-6 text-center text-sm font-medium max-w-sm">
              As per our credit policy, we are currently unable to process your
              loan application.
            </p>
            <button
              onClick={() => {
...
function DashboardContent() {
  ...
  const [accessState, setAccessState] = React.useState<"checking" | "granted">(
    "checking",
  );
  ...
  const [reloanData, setReloanData] = React.useState({
    amount: "",
    appNumber: "",
  });
  ...
  React.useEffect(() => {
    ...
    const onKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") setQrLoan(null);
    };
  ...
  const [addressData, setAddressData] = React.useState([
    {
      id: "addr_1",
      label: "Current address with landmark",
      value: "Not added",
    },
    ...
    async function fetchData() {
      ...
      try {
        ...
        if (response.profile) {
          ...
          const submittedApplications = getSubmittedApplications<any>(
            p.loanApplications,
          );
            ...
            setEmploymentData([
              {
                ...
                value: p.employment.employmentType
                  ? p.employment.employmentType.replace(/_/g, " ")
                  : "-",
              },
              {
                id: "emp_2",
                label: "Company name",
                value:
                  p.employment.employerName === "Self"
                    ? "-"
                    : p.employment.employerName || "-",
              },
              { id: "emp_3", label: "Monthly income", value: incomeDisplay },
              {
                id: "emp_4",
                label: "Stability in current job",
                value: p.employment.stability || "-",
              },
            ]);
          }
            ...
            setAddressData([
              {
                id: "addr_1",
                label: "Current address with landmark",
                value: p.address.currentAddress || "-",
              },
              {
                id: "addr_2",
                label: "Current address type",
                value: p.address.currentAddressType || "-",
              },
              {
                id: "addr_3",
                label: "Permanent address",
                value: p.address.permanentAddress || "-",
              },
            ]);
          }
          ...
          const allData: any[] = submittedApplications.map((app: any) => ({
            id: `APP-${app.id}`,
            rawId: app.id,
            number: formatAppNumber(
              app.id,
              p.aadhaarVerification?.aadhaarNumber,
            ),
          ...
          const profileComplete = !!(
            p.panVerification?.verified && p.aadhaarVerification?.verified
          );
  ...
  const handleUpdateAddress = async () => {
    ...
    try {
      ...
      const data = {
        ...
        currentAddressType:
          addressData.find((i) => i.id === "addr_2")?.value || "",
        permanentAddress:
          addressData.find((i) => i.id === "addr_3")?.value || "",
      };
  ...
  const handleUploadMissingDocs = async () => {
    const filesToUpload = Object.entries(uploadFiles).filter(
      ([_, file]) => !!file,
    ) as [string, File][];
  ...
  const renderDashboardContent = () => (
    <div className="space-y-16">
        ...
        <div className="bg-gradient-to-br from-[#FEF2F2] to-[#FFF7ED] border border-red-100 rounded-3xl p-6 md:p-8 shadow-[0_10px_35px_rgb(239,68,68,0.06)] space-y-6">
          <div className="flex flex-col md:flex-row items-start justify-between gap-6">
            <div className="space-y-2">
              ...
              <p className="text-sm text-gray-600 max-w-2xl leading-relaxed">
                Your application has been created but cannot be processed or
                exported to the LOS team because some required documents are
                missing. Please upload the required documents below to resume
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {missingDocs.map((type) => {
              ...
              return (
                <div
                  key={type}
                  className="border border-dashed border-red-200 bg-white rounded-2xl p-4 flex flex-col justify-between items-center text-center space-y-3 hover:border-red-300 transition-colors"
                >
                  <div>
          ...
          <div className="flex justify-end pt-2">
            <button
              ...
              disabled={
                isUploadingDocs ||
                missingDocs.some((type) => !uploadFiles[type])
              }
      ...
      <section className="max-w-5xl">
        <h2 className="text-xl sm:text-2xl md:text-[28px] font-bold text-[#EF4444] mb-8">
          Personal Details
        </h2>
              ...
              <div className="relative group">
                {detail.id === "emp_4" && editingFields[detail.id] ? (
                  <div className="relative w-full">
                    <select
                      ref={(el) => {
                        inputRefs.current[detail.id] = el as any;
                      }}
                    ...
                    >
                      ...
                      <option value="SOMEWHAT_UNSTABLE">
                        Somewhat Unstable
                      </option>
                    ...
                    <ChevronDown
                      className="absolute right-12 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none"
                      size={20}
                    />
                  </div>
                ) : (
                  <input
                    ref={(el) => {
                      inputRefs.current[detail.id] = el;
                    }}
                    type="text"
                    value={detail.value}
                    onChange={(e) =>
                      handleInputChange(detail.id, e.target.value, "employment")
                    }
                    onBlur={() => handleBlur(detail.id)}
                    readOnly={
                    ...
                    className={`w-full border rounded-2xl px-6 py-4 font-medium outline-none pr-12 transition-all shadow-sm ${
                      editingFields[detail.id] &&
                      detail.id !== "emp_1" &&
                      detail.id !== "emp_3"
            ...
            <div key={detail.id} className="space-y-2.5">
              ...
              <div className="relative group">
                <input
                  ...
                  value={detail.value}
                  onChange={(e) =>
                    handleInputChange(detail.id, e.target.value, "address")
                  }
                  onBlur={() => handleBlur(detail.id)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
        <div className="space-y-2.5">
          ...
          <input
            ...
            onChange={(e) =>
              setSearchQuery((prev) => ({ ...prev, appId: e.target.value }))
            }
        ...
        <div className="space-y-2.5">
          ...
          <input
            ...
            onChange={(e) =>
              setSearchQuery((prev) => ({ ...prev, phone: e.target.value }))
            }
            className="w-full bg-white border border-gray-200 rounded-xl px-6 py-4 text-gray-700 font-medium outline-none focus:ring-1 focus:ring-red-100 transition-all"
          />
        </div>
          ...
          onClick={() => {
            ...
            if (trimmedAppId) {
              matches = matches.filter((l) =>
                l.number.toLowerCase().includes(trimmedAppId.toLowerCase()),
              );
            }
      ...
      {trackResult && (
        <div className="mt-8 space-y-4">
          {trackResult === "NOT_FOUND" ? (
            ...
            trackResult.map((result: any, idx: number) => (
              <div
                key={idx}
              ...
              >
                ...
                <div className="flex justify-between border-b pb-2">
                  ...
                  <span
                    className={`font-bold ${formatApplicationStatus(result.status) === IN_PROCESS_LABEL ? "text-amber-500" : "text-emerald-500"}`}
                  >
            ...
            >
              ...
              <div className="flex flex-col gap-3">
                ...
                <button
                  ...
                  onClick={() =>
                    setBankDetailsLoanId((currentId) =>
                      currentId === loan.id ? null : loan.id,
                    )
                  }
              ...
              {bankDetailsLoanId === loan.id && (
                <div className="md:col-span-4 rounded-2xl border border-gray-100 bg-gray-50 p-5 space-y-3">
                  ...
                  <p className="text-xs font-medium text-gray-500">
                    QR code attached above for quick payment.
                  </p>
                </div>
              )}
            </div>
  ...
  const renderLoanHistoryContent = () => (
    <div className="max-w-5xl space-y-10">
      <div className="flex items-center justify-between">
        <h2 className="text-[28px] font-extrabold text-[#111827] tracking-tight text-[#EF4444]">
          Loan History
        </h2>
      </div>
      ...
      <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-[0_15px_50px_-15px_rgba(0,0,0,0.05)] overflow-hidden">
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="border-b border-gray-50 bg-gray-50/30">
                <th className="px-4 md:px-8 py-4 md:py-5 text-[12px] md:text-[13px] font-bold text-gray-400 uppercase tracking-wider whitespace-nowrap">
                  Loan Reference
                </th>
                <th className="px-4 md:px-8 py-4 md:py-5 text-[12px] md:text-[13px] font-bold text-gray-400 uppercase tracking-wider whitespace-nowrap">
                  Amount
            ...
            <tbody className="divide-y divide-gray-50">
              {loanHistoryData.length > 0 ? (
                loanHistoryData.map((loan) => (
                  <tr
                    key={loan.id}
                    className="group hover:bg-red-50/20 transition-all"
                  >
        ...
        <div className="bg-gray-50/50 px-8 py-4 border-t border-gray-50">
          <p className="text-xs font-medium text-gray-400">
            Showing {loanHistoryData.length} loan records found in your account.
          </p>
        </div>
      </div>
    </div>
  ...
  const renderSupport = () => (
    <div className="space-y-12">
      <h2 className="text-xl sm:text-2xl md:text-[28px] font-bold text-[#EF4444]">
        Connect us
      </h2>
      ...
      <div className="space-y-6 pt-4">
        ...
        <button
          onClick={() =>
            window.open(
              "https://api.whatsapp.com/send/?phone=919217364584&text=Hi%20I%20have%20applied%20for%20a%20loan.%20I%20have%20a%20query.%20Please%20assist&type=phone_number&app_absent=0",
              "_blank",
            )
          }
          ...
          >
            ...
            <div className="px-6 sm:px-8 py-6 space-y-5">
                ...
                <button
                  onClick={async () => {
                    try {
                      ...
                      toast.error(
                        "Could not copy. Please note the UPI ID manually.",
                      );
                    }
                  }}
              ...
              <p className="text-[11px] text-gray-400 font-medium text-center leading-relaxed">
                After paying, keep the UPI transaction reference safe.
                Repayments are updated against your loan within 24 working
                hours.
              </p>
            </div>
          </div>
